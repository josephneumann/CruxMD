...
rich conversational experiences with clinical insights and suggested
follow-ups. The LLM outputs JSON conforming to AgentResponse (narrative,
insights, follow-ups). Tables and visualizations are generated
...
class AgentResponse(BaseModel):
    ...
    elements like insights and follow-up suggestions. Tables and
    visualizations are NOT included here — they are generated
    deterministically by backend code and attached via ChatAgentResponse.
    """
...
class ChatAgentResponse(AgentResponse):
    """API response model — extends LLM output with deterministic tables and visualizations.

    Tables and visualizations are generated by backend code (table_builder.py,
    chart_builder.py), not by the LLM. This model is used for the API response
    only, never as the OpenAI schema.
    """
    ...
    visualizations: list[ClinicalVisualization] | None = Field(
        default=None,
        description="Clinical charts and timelines (deterministically generated by backend)",
    )
