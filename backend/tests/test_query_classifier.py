"""Tests for the adaptive query classifier.

Tests the heuristic classifier that routes queries to FAST (chart lookup)
or STANDARD (clinical reasoning) profiles.
"""
...
class TestClassifyQueryFastPath:
    """All three paths to FAST classification."""
    ...
    @pytest.mark.parametrize("msg", [
        ...
        # Historical lookups (still Path A)
    ...
    def test_path_a_entity_plus_prefix(self, msg: str):
        assert classify_query(msg) == FAST_PROFILE
...
class TestClassifyQueryStandardPath:
    """All paths to STANDARD classification."""
    ...
    def test_reasoning_keywords(self, msg: str):
        assert classify_query(msg) == STANDARD_PROFILE
    ...
    # Summary/overview queries (no chart entity -> STANDARD)
    @pytest.mark.parametrize("msg", [
...
class TestClassifyQueryEdgeCases:
    ...
    def test_empty_string(self):
        assert classify_query("") == STANDARD_PROFILE
    ...
    def test_case_insensitivity(self):
        assert classify_query("WHAT MEDICATIONS?") == FAST_PROFILE
        assert classify_query("What Medications?") == FAST_PROFILE

    def test_reasoning_keyword_overrides_entity(self):
        assert classify_query("Why is the patient on these medications?") == STANDARD_PROFILE
        assert classify_query("Explain the lab results") == STANDARD_PROFILE

    def test_analytical_phrase_overrides_entity(self):
        assert classify_query("Show me encounters and see if he's getting worse") == STANDARD_PROFILE

    def test_contraindication_stem(self):
        assert classify_query("Any contraindications for this drug?") == STANDARD_PROFILE

    def test_no_chart_entity_various(self):
        assert classify_query("Tell me about this patient") == STANDARD_PROFILE
        assert classify_query("Hello") == STANDARD_PROFILE
        assert classify_query("What do you think?") == STANDARD_PROFILE

    def test_how_many_is_fast(self):
        assert classify_query("How many medications?") == FAST_PROFILE

    def test_how_should_is_standard(self):
        assert classify_query("How should we manage the diabetes?") == STANDARD_PROFILE

    def test_follow_up_shorthand_and_the(self):
        # "And the allergies?" -> no lookup prefix -> STANDARD
        assert classify_query("And the allergies?") == STANDARD_PROFILE

    def test_what_about_is_fast(self):
        # "What about the labs?" -> "what" prefix + entity -> FAST
        assert classify_query("What about the labs?") == FAST_PROFILE

    def test_clinical_shorthand_bp(self):
        assert classify_query("bp") == FAST_PROFILE
        assert classify_query("bp?") == FAST_PROFILE

    def test_clinical_shorthand_hr(self):
        assert classify_query("hr") == FAST_PROFILE

    def test_whether_analytical_phrase(self):
        assert classify_query("Show labs and whether the A1c improved") == STANDARD_PROFILE
...
class TestQueryProfileValues:
    ...
    def test_fast_profile_values(self):
        assert FAST_PROFILE.tier == QueryTier.FAST
        assert FAST_PROFILE.reasoning_effort == "low"
        assert FAST_PROFILE.include_tools is False
        assert FAST_PROFILE.max_output_tokens == 4096
        assert FAST_PROFILE.system_prompt_mode == "fast"

    def test_standard_profile_values(self):
        assert STANDARD_PROFILE.tier == QueryTier.STANDARD
        assert STANDARD_PROFILE.reasoning_effort == "medium"
        assert STANDARD_PROFILE.include_tools is True
        assert STANDARD_PROFILE.max_output_tokens == 16384
        assert STANDARD_PROFILE.system_prompt_mode == "standard"

    def test_profiles_are_frozen(self):
        with pytest.raises(AttributeError):
            FAST_PROFILE.tier = QueryTier.STANDARD  # type: ignore[misc]
        with pytest.raises(AttributeError):
            STANDARD_PROFILE.tier = QueryTier.FAST  # type: ignore[misc]
